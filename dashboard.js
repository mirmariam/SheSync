...
async function loadUserData() {
  ...
  try {
    ...
    if (userDoc.exists()) {
      ...
      if (userData.periodDays) {
        markSavedPeriodDays(userData.periodDays);
      }
    }
  } catch (error) {
    console.error('Error loading user data:', error);
...
// Update input fields
function updateInputFields() {
  if (cycleData.lastPeriodDate) document.getElementById('lastPeriodDate').value = cycleData.lastPeriodDate;
  document.getElementById('cycleLength').value = cycleData.cycleLength || 28;
  document.getElementById('periodLength').value = cycleData.periodLength || 5;
}
...
function handleDateClick(info) {
  ...
  if (clickedDate <= today) {
    const isConfirmed = confirm(`Mark ${clickedDate} as a period day?`);
    if (isConfirmed) markPeriodDay(clickedDate);
  }
}

// Mark period day in Firestore
async function markPeriodDay(date) {
  if (!currentUser) return;
  try {
    const userRef = doc(db, 'users', currentUser.uid);
...
// Mark saved period days from Firestore
function markSavedPeriodDays(periodDays) {
  Object.keys(periodDays).forEach(date => {
    if (periodDays[date]) {
      calendar.addEvent({
        title: 'Period',
        start: date,
...
async function updateCycleTracking() {
  ...
  if (!lastPeriodInput) return alert('Please enter your last period start date');

  cycleData = { lastPeriodDate: lastPeriodInput, cycleLength, periodLength };

  if (currentUser) {
    try {
      await updateDoc(doc(db, 'users', currentUser.uid), { cycleData });
    } catch (error) {
      console.error('Error saving cycle data:', error);
    }
  }

  calculateCycleEvents(lastPeriodInput, cycleLength, periodLength);
}
...
function calculateCycleEvents(startDate, cycleLength, periodLength) {
  ...
  for (let i = 0; i < 12; i++) {
    ...
    for (let j = 0; j < periodLength; j++) {
      const periodDate = new Date(cycleStart);
      periodDate.setDate(cycleStart.getDate() + j);
      events.push({ title: 'Period', start: periodDate.toISOString().split('T')[0], display: 'background', className: 'fc-event-period' });
      savePeriodDay(periodDate.toISOString().split('T')[0]);
    }

    // Ovulation
    const ovulationDate = new Date(cycleStart);
    ovulationDate.setDate(cycleStart.getDate() + cycleLength - 14);
    events.push({ title: 'Ovulation', start: ovulationDate.toISOString().split('T')[0], display: 'background', className: 'fc-event-ovulation' });

    // Fertile window (5 days before ovulation)
    for (let k = -4; k <= 0; k++) {
      const fertileDate = new Date(ovulationDate);
      fertileDate.setDate(ovulationDate.getDate() + k);
      if (k !== 0) events.push({ title: 'Fertile', start: fertileDate.toISOString().split('T')[0], display: 'background', className: 'fc-event-fertile' });
    }
    ...
    for (let l = -5; l < 0; l++) {
      const pmsDate = new Date(cycleStart);
      pmsDate.setDate(cycleStart.getDate() + l);
      events.push({ title: 'PMS', start: pmsDate.toISOString().split('T')[0], display: 'background', className: 'fc-event-pms' });
    }
  }
...
// Save period day to Firestore
async function savePeriodDay(date) {
  if (!currentUser) return;
  try {
    const userRef = doc(db, 'users', currentUser.uid);
    await updateDoc(userRef, { [`periodDays.${date}`]: true });
  } catch (error) {
...
async function saveSymptoms() {
  if (!currentUser) return;
  const symptoms = Array.from(document.querySelectorAll('.symptom-check:checked')).map(cb => cb.dataset.symptom);
  const flow = document.getElementById('flowIntensity').value;
  const today = new Date().toISOString().split('T')[0];

  try {
    await updateDoc(doc(db, 'users', currentUser.uid), {
      [`symptoms.${today}`]: { symptoms, flow, timestamp: new Date() }
    });
    alert('Symptoms saved successfully!');
    document.querySelectorAll('.symptom-check').forEach(cb => cb.checked = false);
    document.getElementById('flowIntensity').value = '';
  } catch (error) {
    console.error('Error saving symptoms:', error);
    alert('Error saving symptoms. Please try again.');
...
function calculatePCOSRisk() {
  ...
  questions.forEach(q => {
    ...
    if (sel && sel.value==='yes') score++;
  });

  const resultDiv = document.getElementById('riskResult');
  let riskLevel, message, className;

  if (score<=1) { riskLevel='Low'; message='Your symptoms suggest a low risk for PCOS. Continue maintaining a healthy lifestyle and regular check-ups.'; className='low'; }
  else if (score<=2) { riskLevel='Moderate'; message='You have some symptoms that may be associated with PCOS. Consider discussing these with your healthcare provider.'; className='moderate'; }
  else { riskLevel='High'; message='Multiple symptoms suggest you should consult with a healthcare provider for proper evaluation and potential PCOS screening.'; className='high'; }

  resultDiv.innerHTML = `<h4>Risk Assessment: ${riskLevel}</h4><p>${message}</p>`;
  resultDiv.className = `risk-result ${className}`;
  resultDiv.style.display = 'block';
}

// Water intake
function addWaterGlass() { waterCount++; updateWaterDisplay(); saveWaterCount(); }
function resetWaterCount() { waterCount=0; updateWaterDisplay(); saveWaterCount(); }
function updateWaterDisplay() { document.getElementById('waterCount').textContent = waterCount; }
function saveWaterCount() { localStorage.setItem('waterCount', waterCount); localStorage.setItem('waterDate', new Date().toDateString()); }
function loadWaterCount() {
  const savedDate = localStorage.getItem('waterDate'), today = new Date().toDateString();
  waterCount = savedDate===today ? parseInt(localStorage.getItem('waterCount'))||0 : 0;
  updateWaterDisplay();
  if(savedDate!==today) saveWaterCount();
}

// Sections
function openSection(name) {
  document.querySelectorAll('.feature-card').forEach(sec=>sec.classList.add('hidden'));
...
function openArticle(type) {
  const articles = { /* omitted for brevity: same as your original */ };
  const article = articles[type];
  if(article){ document.getElementById('articleContent').innerHTML = article.content; document.getElementById('articleModal').style.display='block'; }
}
function closeArticleModal(){document.getElementById('articleModal').style.display='none';}

// AI Period Prediction
async function generatePrediction() {
  if(!periodPredictor){ alert('Period predictor not initialized. Please refresh the page.'); return; }

  const lastPeriodInput = document.getElementById('lastPeriodDate');
  const cycleLengthInput = document.getElementById('cycleLength');
  const periodLengthInput = document.getElementById('periodLength');

  if(!lastPeriodInput.value){ alert('Please enter your last period start date first'); return; }

  const userData = {
    lastPeriodDate: lastPeriodInput.value,
    cycleHistory: getCycleHistory(),
    age:28, stressLevel:0.5, bmi:22, exerciseFrequency:0.6
  };
      ...
      try {
        ...
        await updateDoc(userRef,{
          [`predictions.${new Date().toISOString().split('T')[0]}`]: {
            ...
            nextPeriodDate: prediction.nextPeriodDate.toISOString(),
            ovulationDate: prediction.ovulationDate.toISOString(),
            fertileWindow:{ start: prediction.fertileWindow.start.toISOString(), end: prediction.fertileWindow.end.toISOString() },
            timestamp: new Date()
          }
        });
...
function displayPredictionResults(pred){ const resDiv=document.getElementById('predictionResult'); resDiv.style.display='block'; resDiv.querySelector('#predictionContent').innerHTML=`<p>Next Period: ${pred.nextPeriodDate.toDateString()}</p><p>Ovulation: ${pred.ovulationDate.toDateString()}</p><p>Fertile Window: ${pred.fertileWindow.start.toDateString()} - ${pred.fertileWindow.end.toDateString()}</p>`; }

function getCycleHistory(){ /* placeholder: return past period dates array */ return []; }
